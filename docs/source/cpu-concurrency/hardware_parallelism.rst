...
====================
logical*cores = os.cpu*count()  # e.g., 8
...
===============================
Typically: physical*cores = logical*cores / 2 (if HT enabled)
=============================================================
::
...
if platform.system() == 'Darwin':  # macOS
    ...
    physical*cores = int(result.stdout.strip())
elif platform.system() == 'Linux':
    # Count unique physical IDs
    with open('/proc/cpuinfo') as f:
        physical*cores = len(set(
            line.split(':')[1].strip()
            for line in f
            if line.startswith('physical id')
        ))
else:  # Windows
    physical*cores = logical*cores // 2  # Approximation

print(f"Logical cores: {logical*cores}")
print(f"Physical cores: {physical*cores}")
print(f"Hyperthreading: {'Enabled' if logical*cores > physical*cores else 'Disabled'}")
::
...
def benchmark(num*workers):
    start = time.perf*counter()
    with ProcessPoolExecutor(max*workers=num*workers) as executor:
        tasks = [10*000*000] * num*workers
        list(executor.map(compute, tasks))
    return time.perf*counter() - start
...
Best practice:
==============
physical*cores = os.cpu*count() // 2  # Approximate physical cores
optimal*workers = physical*cores

Conservative (recommended for production):
==========================================
optimal*workers = max(1, physical*cores - 1)  # Leave one core for OS

Or detect actual physical cores:
================================
import psutil  # pip install psutil
optimal*workers = psutil.cpu*count(logical=False)
::
...
def multiply*matrices(size):
    A = np.random.rand(size, size)
    B = np.random.rand(size, size)
    return np.dot(A, B)

def benchmark(num*workers, num*tasks=12):
    """
    ...
    with ProcessPoolExecutor(max*workers=num*workers) as executor:
        futures = [executor.submit(multiply*matrices, 1000)
                   for * in range(num*tasks)]
        results = [f.result() for f in futures]

    elapsed = time.perf*counter() - start
    speedup = (num*tasks * 1.0) / elapsed
    efficiency = speedup / num*workers * 100
...
Time: 250,000 additions ÷ core*speed
::
...
4. **Scientific Simulations**
   ...
   for particle*i in particles:
       force = sum(calculate*force(particle*i, particle*j)
                   for particle*j in other*particles)
...
1. **Complex Control Flow**
   ...
   def complex*logic(data):
       if data.type == 'A':
           if data.value > threshold*1:
               return process*A1(data)
           else:
               return process*A2(data)
       elif data.type == 'B':
           # ... more branching
...
3. **Database Queries with Joins**
   ...
   SELECT * FROM table1
   JOIN table2 ON table1.id = table2.foreign*id
   WHERE complex*condition(table1.data)
...
print(f"CPU time: {cpu*time:.3f}s")
...
start = time.time()
C*gpu = cp.dot(A*gpu, B*gpu)
cp.cuda.Stream.null.synchronize()  # Wait for GPU
gpu*time = time.time() - start
print(f"GPU time: {gpu*time:.3f}s")

print(f"Speedup: {cpu*time / gpu*time:.1f}x")
...
====================================
import cv2
for img*path in image*paths:
    img = cv2.imread(img*path)
    blurred = cv2.GaussianBlur(img, (15, 15), 0)
    cv2.imwrite(output*path, blurred)
Time: ~300 seconds
==================
...
def process*image(img*path):
    img = cv2.imread(img*path)
    blurred = cv2.GaussianBlur(img, (15, 15), 0)
    cv2.imwrite(output*path, blurred)

with ProcessPoolExecutor(max*workers=4) as executor:
    executor.map(process*image, image*paths)
...
def process*image*gpu(img*path):
    img = cv2.imread(img*path)
    img*gpu = cp.array(img)
    blurred*gpu = ndimage.gaussian*filter(img*gpu, sigma=3)
    blurred = cp.asnumpy(blurred*gpu)
    cv2.imwrite(output*path, blurred)

for img*path in image*paths:
    process*image*gpu(img*path)
...
start = time.time()
pi*estimate = estimate*pi*cpu(1*000*000*000)
...
def estimate*pi*parallel(n, num*workers=4):
    chunk*size = n // num*workers
    with ProcessPoolExecutor(max*workers=num*workers) as executor:
        results = executor.map(estimate*pi*cpu, [chunk*size] * num*workers)
    return sum(results) / num*workers

start = time.time()
pi*estimate = estimate*pi*parallel(1*000*000*000, 4)
...
start = time.time()
pi*estimate = estimate*pi*gpu(1*000*000*000)
...
optimizer = torch.optim.Adam(model*cpu.parameters())
...
for epoch in range(10):
    for batch in train*loader:
        ...
        loss = nn.functional.cross*entropy(output, target)
        loss.backward()
        optimizer.step()

cpu*time = time.time() - start
print(f"CPU training time: {cpu*time:.2f}s")
Output: CPU training time: 450.00s
==================================

GPU Training
============
model*gpu = model.to('cuda')
optimizer = torch.optim.Adam(model*gpu.parameters())
...
gpu*time = time.time() - start
print(f"GPU training time: {gpu*time:.2f}s")
print(f"Speedup: {cpu*time / gpu_time:.1f}x")
