...
double __nv*tgamma(double)     // Gamma function
::
...
def asin(x):
    if x.dtype == tl.float32:
        ...
        return __nv*asin(x)   # Double precision
    # ... etc for other types
::
...
def asin_triton(x):
    ...
    asin_kernel`grid <x, output, n_elements, BLOCK_SIZE=1024>`_
    return output
...
Error: undefined reference to '**nv_asin'
::
...
extern_libs = get_extern*libs()
kernel`grid <..., extern_libs=extern_libs>`_
::
