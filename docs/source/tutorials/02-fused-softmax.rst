...
softmax(x*i) = exp(x*i - max(x)) / Σ exp(x*i - max(x))
::
...
x*max = x.max(dim=1)[0]           # Read MN, write M
z = x - x*max[:, None]             # Read MN+M, write MN
...
mask = col*offsets < n*cols
row = tl.load(input*ptrs, mask=mask, other=-float('inf'))
::


**Important Details**:
- ``BLOCK*SIZE`` must be power of 2 (GPU requirement)
- If ``n*cols < BLOCK*SIZE``, we pad with ``-inf``
...
**GPU Math Functions**:
- Uses hardware-accelerated exp (CUDA ``_*expf``)
...
tl.store(output*ptrs, softmax*output, mask=mask)
::
...
Why Must BLOCK*SIZE Be Power of 2?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
2. **Warp Operations**:
   ...
   - Efficient operations require BLOCK*SIZE % 32 == 0
...
For example:
- n*cols = 1000 → BLOCK*SIZE = 1024
- n*cols = 513 → BLOCK*SIZE = 1024
- n*cols = 512 → BLOCK*SIZE = 512
...
num*stages = 4 if SIZE*SMEM > 200000 else 2
::
...
occupancy = NUM*REGS // (n*regs * WARP*SIZE * num*warps)
occupancy = min(occupancy, SIZE*SMEM // size*smem)
num*programs = NUM*SM * occupancy
::
...
- NUM*SM = 108 (streaming multiprocessors)
- NUM*REGS = 65536 registers per SM
- SIZE*SMEM = 164 KB per SM
- num*warps = 8, WARP*SIZE = 32
...
for row*idx in tl.range(row*start, n*rows, row*step, num*stages=num*stages):
    # process row*idx
::


Instead of launching one kernel per row:
- Launch ``num*programs`` kernels total
...
- ``**shfl*down*sync()``: Shuffle data from higher-indexed thread
...
- You think in terms of vectors (BLOCK*SIZE elements)
...
~~~~~~~~~~~~~~~~~~~~~~~~~
If ``BLOCK*SIZE * sizeof(float32) > SRAM*SIZE``:
...
1. **Ensure rows fit in SRAM**: Check ``BLOCK*SIZE * 4 < SRAM*SIZE``
2. **Use float32 for accumulation**: Better numerical accuracy
3. **Tune num*warps**: Try 4, 8, or 16 depending on row size
4. **Adjust num*stages**: More stages hide latency but need more SRAM
