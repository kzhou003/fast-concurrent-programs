...
def put(self, item):
    ...
    with self.mutex:
        ...
        # Wakes one consumer waiting on not_empty
...
def get(self):
    ...
    with self.mutex:
        ...
        # Wakes one producer waiting on not_full
...
class GoodQueue:
    def __init__(self):
        self.items = []
        self.not_empty = Condition()  # Single condition for ALL items

    def put(self, item):
        self.items.append(item)  # Item is just data
        self.not_empty.notify()  # Signal all waiting consumers
...
2. **Queue has conditions** - not_empty, not_full
...
5. **put() signals not_empty** - "Item added, consumers can proceed"
6. **get() signals not_full** - "Space freed, producers can proceed"
