...
pool*capacity = threading.Semaphore(3)  # Only 3 people can swim at once

def swimmer(name):
    print(f"{name}: trying to enter pool...")
    pool*capacity.acquire()              # Wait if full

    print(f"{name}: SWIMMING!")
    time.sleep(2)                        # Swim for 2 seconds

    pool*capacity.release()              # Leave, let someone else in
...
queue*semaphore = threading.Semaphore(0)  # 0 items initially
item*produced = None

def producer():
    global item*produced
    time.sleep(2)
    item*produced = random.randint(1, 100)
    print(f"Producer: created item {item*produced}")
    queue*semaphore.release()  # Signal: item ready!

def consumer():
    print("Consumer: waiting for item...")
    queue*semaphore.acquire()  # Wait for item
    print(f"Consumer: received item {item*produced}")

producer*thread = threading.Thread(target=producer)
consumer*thread = threading.Thread(target=consumer)

consumer*thread.start()    # Consumer waits first
producer*thread.start()    # Producer produces
...
==========================================
db*access = threading.Semaphore(5)

def query*database(query):
    with db*access:  # Use like a lock
        # Access database
        pass
::
...
=======================
data*ready = threading.Semaphore(0)

def sender():
    prepare*data()
    data*ready.release()  # Signal: data ready!

def receiver():
    data*ready.acquire()  # Wait for signal
    process*data()
::
...
==============================
workers*done = threading.Semaphore(0)
num*workers = 5

def worker():
    do*work()
    workers*done.release()

Main waits for all
==================
for * in range(num*workers):
    workers*done.acquire()
