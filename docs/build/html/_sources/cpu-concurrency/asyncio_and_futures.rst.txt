...
File Location
-------------
``basics/10*asyncio*and*futures.py``
...
async def second*coroutine(num):
    count = 1
...
async def main():
    ...
    tasks = [
        asyncio.create*task(first*coroutine(num1)),
        asyncio.create*task(second*coroutine(num2))
    ]
...
4. **Removed ``add*done*callback()`` pattern**
   - Old: ``future.add*done*callback(got*result)``
   - New: Direct return values and ``await``
   - Callbacks are unnecessary with modern async/await

5. **Removed ``future.set*result()`` pattern**
   - Old: Manually setting future results
   - New: Simply return values from async functions

6. **Simplified to ``asyncio.create*task()`` and ``asyncio.gather()``**
...
Or with tasks:
==============
task = asyncio.create*task(coroutine(value))
result = await task
::
...
Reads command-line arguments
  ↓
Creates 2 tasks:
  ├─→ first*coroutine(num1)
  └─→ second*coroutine(num2)
...
python3 10*asyncio*and*futures.py <num1> <num2>
::
...
python3 10*asyncio*and*futures.py 10 5
::
...
python3 10*asyncio*and*futures.py 100 7
::
...
2. **Use ``asyncio.create*task()``** to schedule coroutines
...
async def main():
    tasks = [
        asyncio.create*task(first*coroutine(num1)),
        asyncio.create*task(second_coroutine(num2))
    ]
