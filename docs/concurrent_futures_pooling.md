...
### Changes Made
1. Replaced deprecated `time.clock()` with `time.perf_counter()`
   - `time.clock()` was removed in Python 3.8
   - `time.perf_counter()` provides higher resolution and is the recommended timing function
...
## Usage
```bash
python3 06_concurrent_futures_pooling.py
```
