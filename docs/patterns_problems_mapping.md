...
## Table of Contents
1. [The Fundamental Problem: The GIL](#the-fundamental-problem-the-gil)
2. [How CPU and I/O Operations Differ](#how-cpu-and-io-operations-differ)
3. [Why Multiprocessing for CPU-bound](#why-multiprocessing-for-cpu-bound)
4. [Why Threading for I/O-bound](#why-threading-for-io-bound)
5. [Why Asyncio for I/O-bound](#why-asyncio-for-io-bound)
6. [Deep Dive: What Happens Under the Hood](#deep-dive-what-happens-under-the-hood)
...
with ProcessPoolExecutor(max_workers=2) as executor:
    futures = [executor.submit(cpu_work) for _ in range(2)]
...
t1 = threading.Thread(target=compute_sum, args=(10_000_000,))
t2 = threading.Thread(target=compute_sum, args=(10_000_000,))
```
...
with ProcessPoolExecutor(max_workers=2) as executor:
    future1 = executor.submit(compute_sum, 10_000_000)
    future2 = executor.submit(compute_sum, 10_000_000)
```
...
async def process_data(session, url, executor):
    ...
    result = await loop.run_in_executor(executor, complex_computation, data)
    return result
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        results = executor.map(io_intensive_task, urls)
...
async def io_intensive_task(session, url):
    async with session.get(url) as response:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [io_intensive_task(session, url) for url in urls]
        results = await asyncio.gather(*tasks)
