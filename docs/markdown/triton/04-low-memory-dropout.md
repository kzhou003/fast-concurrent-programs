...
Need `preserve_rng_state=True` → more complexity and overhead.
...
```
random_value = hash(seed, counter)
```
...
class Dropout(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            return seeded_dropout(x, self.p, self.seed)
        return x
```
...
As always, handle non-multiple-of-BLOCK_SIZE tensors safely.
...
class SeededDropout(torch.autograd.Function):
    ...
    def forward(ctx, x, p, seed):
        output = seeded_dropout_triton(x, p, seed)
    ...
    def backward(ctx, grad_output):
        ...
        dropout_mask = generate_mask_triton(x.shape, ctx.p, ctx.seed)
        grad_x = grad_output * dropout_mask / (1 - ctx.p)
        return grad_x, None, None  # No gradients for p and seed
```
