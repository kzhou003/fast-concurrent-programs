...
**Process**:
1. Initialize accumulator `_mean`
...
```
Iteration 0: Load x[0:256], add to _mean
Iteration 1: Load x[256:512], add to _mean
Iteration 2: Load x[512:768], add to _mean
Iteration 3: Load x[768:1000], add to _mean (only 232 elements, rest masked)
Final: sum(_mean) / 1000
```
...
```python
GROUP_SIZE_M = 64  # Rows per group
row = tl.program_id(0)
group_id = row // GROUP_SIZE_M
```

Example (M=256 rows, GROUP_SIZE_M=64):
```
...
```python
def _layer_norm_bwd_dwdb(DW, DB, FINAL_DW, FINAL_DB, M, N, GROUP_SIZE_M):
    num_groups = triton.cdiv(M, GROUP_SIZE_M)
...
3. **Adjust GROUP_SIZE_M**: Balance parallelism vs lock contention
