...
def task_done(self):
    with self.mutex:
        ...
        if self.unfinished_tasks == 0:
            self.all_tasks_done.notify_all()  # Wake up join()
```
...
2. Check if counter = 0, then notify all_tasks_done
   ┌──────────────┐
   │ counter: 1   │
   └──────┬───────┘
          │ task_done()
...
## The Third Condition Variable: all_tasks_done
...
class Queue:
    def __init__(self):
        ...
        self.all_tasks_done = Condition() # task_done() signals, join() waits
        self.items = []
...
def task_done(self):
    with self.mutex:
        ...
        if self.unfinished_tasks == 0:
            self.all_tasks_done.notify_all()  # ← Notify join()!
```
...
def join(self):
    with self.all_tasks_done:
        while self.unfinished_tasks:
            self.all_tasks_done.wait()  # ← Waits here!
...
├─ if unfinished_tasks == 0:
│   ├─ YES! (now 0)
│   └─ all_tasks_done.notify_all()  ← Signal join()!
│
├─ State now:
│  ┌──────────────────────┐
...
├─ Acquire all_tasks_done condition
│
├─ while unfinished_tasks != 0:
│   └─ all_tasks_done.wait()  (waiting...)
...
2. COMPLETION FLOW:
   ...
   Signal: all_tasks_done (join() waits for completion)
...
│ └─────────────────────────────────────┘│
│                                        │
│ all_tasks_done Condition Variable      │
│ ┌─────────────────────────────────────┐│
...
| `task_done()` | all_tasks_done | Notifies join() |
| `join()` | unfinished_tasks | Checks if 0 |
| `join()` | all_tasks_done | Waits on it |
...
## The Flow in Your Code (05_queue.py)
...
3. **Notifies all_tasks_done condition variable** (if counter = 0)
