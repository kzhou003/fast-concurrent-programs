...
class _attention(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        PRE_BLOCK = 128
        # Reduced NUM_STAGES from 5 to 3 to fit shared memory limit on RTX 3070 Ti
...
TORCH_HAS_FP8 = hasattr(torch, 'float8_e5m2')
# Reduced for 8GB GPU - original was BATCH=4, N_HEADS=32
BATCH, N_HEADS = 2, 16
            ...
            for warp_specialize in [False, True] if enable_ws else [False]:
                configs.append(
                    triton.testing.Benchmark(
                        x_names=["N_CTX"],
                        # Reduced max sequence length from 2^14 (16384) to 2^12 (4096) for 8GB GPU
